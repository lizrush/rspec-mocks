module RSpec
  module Mocks
    ...
    RSpec.describe 'when verify_partial_doubles configuration option is set' do
      ...
      it 'wraps proxies in a reentrant mutex so we dont trigger deadlocks' do
        klass = Class.new { def message; end; }
        called_for = []
        RSpec.configuration.mock_with(:rspec) do |config|
        ...
        expect { allow(klass.new).to receive(:message) }.to_not raise_error
      end
